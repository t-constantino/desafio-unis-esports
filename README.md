#  Desafio UNIS eSports 2026

## Sobre o Projeto

O Desafio UNIS eSports 2026 é uma iniciativa acadêmica desenvolvida pelos estudantes da Faculdade UNIS com o objetivo de promover a integração entre tecnologia, educação e esportes eletrônicos.

Além da competição, o evento busca conscientizar os participantes sobre temas relacionados à segurança digital, bem-estar, educação e tecnologia, por meio de palestras e atividades interativas.

---

# Objetivos

* Incentivar o trabalho em equipe e o espírito competitivo saudável.
* Promover a conscientização sobre segurança na internet.
* Aproximar estudantes do ensino médio do ambiente acadêmico e tecnológico.
* Desenvolver habilidades de organização, gestão de projetos e comunicação.
* Integrar diferentes áreas do conhecimento em um único evento.

---

# Organização do Evento

*Data:* 13 de junho de 2026

*Horário:* 09h00 às 12h00

 *Local:* Faculdade UNIS

As inscrições serão realizadas através do portal:

*<https://unisesports.com.br>*

As equipes deverão ser formadas previamente pelos próprios participantes.

Cada equipe deverá possuir:

* 4 jogadores titulares;
* 1 jogador reserva (opcional conforme disponibilidade da escola).

Os participantes deverão levar seus próprios dispositivos móveis para participação no campeonato.

---

# Programação

## Abertura do Evento

* Recepção dos participantes;
* Credenciamento das equipes;
* Apresentação das regras gerais.

## Ciclo de Palestras

Serão realizadas palestras multidisciplinares abordando temas como:

### Segurança Digital

* Uso seguro da internet;
* Proteção de dados pessoais;
* Boas práticas no ambiente digital.

### Psicologia



### Educação Física



---

# Campeonato de Free Fire

O campeonato será realizado utilizando o jogo Free Fire.

Participarão representantes de, no mínimo, 8 escolas convidadas.

## Formato da Competição

* Sistema eliminatório;
* Uma equipe representante por escola;
* Equipes compostas por 4 integrantes;
* Progressão até a fase final;
* As duas equipes finalistas disputarão a partida decisiva para definição da campeã.

---

# Regulamento das Competições

As partidas serão acompanhadas pela equipe organizadora para garantir um ambiente justo, seguro e respeitoso para todos os participantes.

## Regras Gerais

* Respeito obrigatório entre todos os participantes;
* Proibido qualquer tipo de ofensa ou comportamento antidesportivo;
* O descumprimento das regras poderá resultar em advertência ou desclassificação.

## Falhas Técnicas

Caso ocorram problemas que comprometam a igualdade da competição relacionados a:

* Conexão Wi-Fi disponibilizada pela instituição;
* Instabilidade da rede durante a partida;
* Funcionamento dos dispositivos dos jogadores;

A partida poderá ser interrompida e reiniciada pela organização sempre que necessário.
O prazo para a contestação dos resultados de uma partida será de 10 minutos após o fim da mesma.

---

# Premiação

Equipe Campeã

* R$ 1.000,00 em premiação.
* 4 medalhas
* 1 troféu

Equipe em Segundo Lugar

* R$ 500,00 em premiação.
* 4 medalhas
* 1 troféu

Equipe em Terceiro Lugar

* 4 medalhas
* 1 troféu

---

# Equipes do Projeto

O projeto foi desenvolvido por estudantes das áreas de:

* Infraestrutura;
* Segurança e Conformidade;
* UX/UI;
* Desenvolvimento;
* Marketing;
* Documentação e Planejamento;
* Organização de Eventos.

---

# Tecnologias e Ferramentas Utilizadas

* ClickUp (Gestão do Projeto)
* GitHub (Versionamento)
* Google Workspace
* Canva
* Microsoft Office
* Ferramentas de Comunicação Digital

---

# Repositório

Este repositório concentra toda a documentação, planejamento, materiais de divulgação, registros das atividades e demais artefatos produzidos durante o desenvolvimento do projeto Desafio UNIS eSports 2026.

---

# Licença

Projeto acadêmico desenvolvido para fins educacionais na Faculdade UNIS.
